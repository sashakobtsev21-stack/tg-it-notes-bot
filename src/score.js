...
Хорошо заходит — реальные примеры из практики этого канала:
- Конфликт/сравнение: "Opus 5 в разы устойчивее конкурентов к prompt injection"
- Дерзкий хак: "разработчик обманул мастер настройки PostHog и получил
  бесплатный безлимитный доступ к Claude"
- Драма/провал: "нейросеть выдумала полсотни несуществующих уязвимостей
  в SQLite, а регуляторы дали им максимальный уровень критичности"
- Крупный релиз с понятным следствием: "Claude Code сделал Opus 5 моделью
  по умолчанию — быстрее и дешевле"

Плохо заходит:
- Рутинный патч-релиз (баг-фиксы, bump версий зависимостей).
- Слишком нишевая специфика — тема реальная и не рутинная, но требует
  бэкграунда в узкой подобласти, чтобы вообще понять, о чём речь (пример:
  "Rust-замена Spark" — понятна только со знанием конкретного data-стека).
- ВАЖНО, отдельная и частая ошибка — "впечатляюще, но сухо": формально
  значимое достижение БЕЗ истории, конфликта, удивления или практического
  следствия для читателя. Живой промах в скоринге: "OpenAI формализовали
  10 доказательств теорем на языке Lean 4" — реальное и некоторым важное
  достижение, но нет зацепки для обычного читателя: не с кем сравнить,
  нечему удивиться, нет ответа на "и что мне с этого". Такое — ниже
  проходного балла, даже если тема правдива и не рутинна.

Оцени тему от 0 (совсем не интересно/непонятно широкой аудитории) до 10
(есть история, конфликт, удивление или явная практическая польза — а не
просто "это важно"). Если сомневаешься между "значимо" и "цепляет" — цени
именно "цепляет".`;
