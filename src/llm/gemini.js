...
async function callGemini(model, generationConfig, { system, user }) {
  ...
  // Живой баг: обрыв по лимиту токенов раньше тихо уходил как обычный текст —
  // владелец получил пост, обрывающийся на полуслове, без ссылки/эмодзи/
  // подписи, потому что до них модель просто не досчитала. Лучше явная
  // ошибка (вызывающий код её уже умеет ловить и пропускать тему), чем
  // заведомо битый черновик молча уйдёт на вычитку.
  if (candidate?.finishReason === "MAX_TOKENS") {
    throw new Error("Ответ модели оборвался по лимиту токенов (MAX_TOKENS)");
  }
  return text.trim();
}
...
// поэтому просто даём запасу токенов больше, чем нужно на ответ. 2048
// оказалось всё равно мало: живой обрыв текста на полуслове ("...максима")
// после того, как промпт разросся новыми правилами (тире/эмодзи/аудитория) —
// думанья стало больше, а лимит остался прежним. Подняли с запасом.
//
...
export async function generateText({ system, user, maxOutputTokens = 4096, temperature = 0.9 }) {
  try {
